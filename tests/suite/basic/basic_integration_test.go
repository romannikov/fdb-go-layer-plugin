...
func TestIntegration_CreateAndGetUser(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		...
		return recordStore.CreateUser(tr, dir, &store.User{Id: "u1", Name: "Alice", Email: "alice@test.com"})
	})

	var user *store.User
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		user, err = recordStore.GetUser(tr, dir, "u1")
		return err
	})
...
func TestIntegration_CreateAndGetProduct(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		...
		return recordStore.CreateProduct(tr, dir, &store.Product{Id: "p1", Name: "Widget", Category: "tools", Price: 42})
	})

	var product *store.Product
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		product, err = recordStore.GetProduct(tr, dir, "p1")
		return err
	})
...
func TestIntegration_GetUser_NotFound(t *testing.T) {
	...
	_, err := db.Transact(func(tr fdb.Transaction) (interface{}, error) {
		return recordStore.GetUser(tr, dir, "nonexistent")
	})
	if err == nil {
...
func TestIntegration_SetUser_UpdateFields(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		...
		return recordStore.CreateUser(tr, dir, &store.User{Id: "u1", Name: "Alice", Email: "a@test.com"})
	})

	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		return recordStore.SetUser(tr, dir, &store.User{Id: "u1", Name: "Bob", Email: "b@test.com"})
	})
...
func TestIntegration_DeleteUser(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		return recordStore.DeleteUser(tr, dir, "u1")
	})

	_, err := db.Transact(func(tr fdb.Transaction) (interface{}, error) {
		return recordStore.GetUser(tr, dir, "u1")
	})
	if err == nil {
...
func TestIntegration_GetUserByEmail(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		...
		if err := recordStore.CreateUser(tr, dir, &store.User{Id: "u1", Name: "Alice", Email: "alice@test.com"}); err != nil {
			return err
		}
		if err := recordStore.CreateUser(tr, dir, &store.User{Id: "u2", Name: "Bob", Email: "bob@test.com"}); err != nil {
			return err
		}
		return recordStore.CreateUser(tr, dir, &store.User{Id: "u3", Name: "Charlie", Email: "alice@test.com"})
	})
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		users, err = recordStore.GetUserByEmail(tr, dir, "alice@test.com")
		return err
	})
	if len(users) != 2 {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		users, err = recordStore.GetUserByEmail(tr, dir, "bob@test.com")
		return err
	})
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		users, err = recordStore.GetUserByEmail(tr, dir, "nobody@test.com")
		return err
	})
	if len(users) != 0 {
...
func TestIntegration_GetProductByCategory(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		...
		if err := recordStore.CreateProduct(tr, dir, &store.Product{Id: "p1", Name: "Hammer", Category: "tools", Price: 15}); err != nil {
			return err
		}
		if err := recordStore.CreateProduct(tr, dir, &store.Product{Id: "p2", Name: "Drill", Category: "tools", Price: 80}); err != nil {
			return err
		}
		return recordStore.CreateProduct(tr, dir, &store.Product{Id: "p3", Name: "Laptop", Category: "electronics", Price: 999})
	})

	var products []*store.Product
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		products, err = recordStore.GetProductByCategory(tr, dir, "tools")
		return err
	})
	if len(products) != 2 {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		products, err = recordStore.GetProductByCategory(tr, dir, "electronics")
		return err
	})
...
func TestIntegration_SetUser_IndexUpdated(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		...
		return recordStore.CreateUser(tr, dir, &store.User{Id: "u1", Name: "Alice", Email: "old@test.com"})
	})

	// Update email
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		return recordStore.SetUser(tr, dir, &store.User{Id: "u1", Name: "Alice", Email: "new@test.com"})
	})
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		users, err = recordStore.GetUserByEmail(tr, dir, "old@test.com")
		return err
	})
	if len(users) != 0 {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		users, err = recordStore.GetUserByEmail(tr, dir, "new@test.com")
		return err
	})
...
func TestIntegration_ListUser_Basic(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		...
		for i := 0; i < 5; i++ {
			...
			if err := recordStore.CreateUser(tr, dir, &store.User{
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		result, err = recordStore.ListUser(tr, dir, store.UserPaginationOptions{Limit: 10})
		return err
	})
...
func TestIntegration_ListUser_Pagination(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		page1, err = recordStore.ListUser(tr, dir, store.UserPaginationOptions{Limit: 2})
		return err
	})
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		page2, err = recordStore.ListUser(tr, dir, store.UserPaginationOptions{Begin: page1.NextKey, Limit: 2})
		return err
	})
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		page3, err = recordStore.ListUser(tr, dir, store.UserPaginationOptions{Begin: page2.NextKey, Limit: 2})
		return err
	})
...
func TestIntegration_ListProduct_Pagination(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		...
		for i := 0; i < 7; i++ {
			if err := recordStore.CreateProduct(tr, dir, &store.Product{
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		page1, err = recordStore.ListProduct(tr, dir, store.ProductPaginationOptions{Limit: 3})
		return err
	})
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		page2, err = recordStore.ListProduct(tr, dir, store.ProductPaginationOptions{Begin: page1.NextKey, Limit: 3})
		return err
	})
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		page3, err = recordStore.ListProduct(tr, dir, store.ProductPaginationOptions{Begin: page2.NextKey, Limit: 3})
		return err
	})
	if len(page3.Items) != 1 {
...
func TestIntegration_BatchGetUser(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		...
		for _, id := range []string{"u1", "u2", "u3"} {
			if err := recordStore.CreateUser(tr, dir, &store.User{Id: id, Name: "Name" + id, Email: id + "@test.com"}); err != nil {
				return err
			}
		}
		return nil
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		var err error
		result, err = recordStore.BatchGetUser(tr, dir, []tuple.Tuple{{"u1"}, {"u2"}, {"u3"}})
		return err
	})
	if len(result) != 3 {
...
func TestIntegration_GenericRepository(t *testing.T) {
	...
	tests.WithTx(t, db, func(tr fdb.Transaction) error {
		return genRepo.Create(tr, dir, &store.User{Id: "g1", Name: "Generic Alice", Email: "g-alice@test.com"})
	})
