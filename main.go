...
import (
	"context"
	{{if hasMutationFields .}}"encoding/binary"{{end}}
	"fmt"
...
func (s *RecordStore) Create{{.Name}}(ctx context.Context, tr Transaction, dir directory.DirectorySubspace, entity *{{.Name}}) error {
	if err := ctx.Err(); err != nil {
		return err
	}
...
func (s *RecordStore) Get{{.Name}}(ctx context.Context, tr fdb.ReadTransaction, dir directory.DirectorySubspace, {{range $index, $element := .PrimaryKeyFields}}{{if $index}}, {{end}}{{.Name}} {{.Type}}{{end}}) (*{{.Name}}, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	typeID, err := s.get{{.Name}}TypeID()
...
func (s *RecordStore) Set{{.Name}}(ctx context.Context, tr Transaction, dir directory.DirectorySubspace, entity *{{.Name}}) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	typeID, err := s.get{{.Name}}TypeID()
...
func (s *RecordStore) Delete{{.Name}}(ctx context.Context, tr Transaction, dir directory.DirectorySubspace, {{range $index, $element := .PrimaryKeyFields}}{{if $index}}, {{end}}{{.Name}} {{.Type}}{{end}}) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	typeID, err := s.get{{.Name}}TypeID()
...
func (s *RecordStore) {{if eq .MutationValue 2}}Add{{else if eq .MutationValue 12}}Max{{else if eq .MutationValue 13}}Min{{end}}{{$.Name}}{{.Name}}(ctx context.Context, tr Transaction, dir directory.DirectorySubspace, {{range $.PrimaryKeyFields}}{{.Name}} {{.Type}}, {{end}} val {{.Type}}) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	typeID, err := s.get{{$.Name}}TypeID()
...
func (s *RecordStore) Get{{$.Name}}By{{joinFieldNames $idx.Fields}}(ctx context.Context, tr fdb.ReadTransaction, dir directory.DirectorySubspace, {{range $i, $f := $idx.Fields}}{{if $i}}, {{end}}{{$f.Name}} {{$f.Type}}{{end}}) ([]*{{$.Name}}, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	typeID, err := s.get{{$.Name}}TypeID()
...
func (s *RecordStore) BatchGet{{.Name}}(ctx context.Context, tr fdb.ReadTransaction, dir directory.DirectorySubspace, ids []tuple.Tuple) (map[string]*{{.Name}}, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	typeID, err := s.get{{.Name}}TypeID()
...
func (s *RecordStore) List{{.Name}}(ctx context.Context, tr fdb.ReadTransaction, dir directory.DirectorySubspace, opts {{.Name}}PaginationOptions) (*{{.Name}}PaginatedResult, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	typeID, err := s.get{{.Name}}TypeID()
...
type {{.Name}}Repository interface {
	GenericRepository[*{{.Name}}, {{if eq (len .PrimaryKeyFields) 1}}{{(index .PrimaryKeyFields 0).Type}}{{else if gt (len .PrimaryKeyFields) 1}}{{.Name}}PrimaryKey{{else}}struct{}{{end}}]
	...
	{{range .Fields}}
	{{if .MutationValue}}
	{{if eq .MutationValue 2}}Add{{else if eq .MutationValue 12}}Max{{else if eq .MutationValue 13}}Min{{end}}{{$.Name}}{{.Name}}(ctx context.Context, tr Transaction, dir directory.DirectorySubspace, {{range $.PrimaryKeyFields}}{{.Name}} {{.Type}}, {{end}} val {{.Type}}) error
	{{end}}
	{{end}}
}
...
func (r *{{.Name | lower}}Repository) Create(ctx context.Context, tr Transaction, dir directory.DirectorySubspace, entity *{{.Name}}) error {
	return r.store.Create{{.Name}}(ctx, tr, dir, entity)
}
...
func (r *{{.Name | lower}}Repository) Get(ctx context.Context, tr fdb.ReadTransaction, dir directory.DirectorySubspace, pk {{if eq (len .PrimaryKeyFields) 1}}{{(index .PrimaryKeyFields 0).Type}}{{else if gt (len .PrimaryKeyFields) 1}}{{.Name}}PrimaryKey{{else}}struct{}{{end}}) (*{{.Name}}, error) {
	return r.store.Get{{.Name}}(ctx, tr, dir, {{if eq (len .PrimaryKeyFields) 1}}pk{{else if gt (len .PrimaryKeyFields) 1}}{{range $i, $f := .PrimaryKeyFields}}{{if $i}}, {{end}}pk.{{$f.Name}}{{end}}{{end}})
}

func (r *{{.Name | lower}}Repository) Set(ctx context.Context, tr Transaction, dir directory.DirectorySubspace, entity *{{.Name}}) error {
	return r.store.Set{{.Name}}(ctx, tr, dir, entity)
}

func (r *{{.Name | lower}}Repository) Delete(ctx context.Context, tr Transaction, dir directory.DirectorySubspace, pk {{if eq (len .PrimaryKeyFields) 1}}{{(index .PrimaryKeyFields 0).Type}}{{else if gt (len .PrimaryKeyFields) 1}}{{.Name}}PrimaryKey{{else}}struct{}{{end}}) error {
	return r.store.Delete{{.Name}}(ctx, tr, dir, {{if eq (len .PrimaryKeyFields) 1}}pk{{else if gt (len .PrimaryKeyFields) 1}}{{range $i, $f := .PrimaryKeyFields}}{{if $i}}, {{end}}pk.{{$f.Name}}{{end}}{{end}})
}
...
func (r *{{.Name | lower}}Repository) BatchGet{{.Name}}(ctx context.Context, tr fdb.ReadTransaction, dir directory.DirectorySubspace, ids []tuple.Tuple) (map[string]*{{.Name}}, error) {
	return r.store.BatchGet{{.Name}}(ctx, tr, dir, ids)
}
...
func (r *{{.Name | lower}}Repository) List{{.Name}}(ctx context.Context, tr fdb.ReadTransaction, dir directory.DirectorySubspace, opts {{.Name}}PaginationOptions) (*{{.Name}}PaginatedResult, error) {
	return r.store.List{{.Name}}(ctx, tr, dir, opts)
}
...
func (r *{{$.Name | lower}}Repository) Get{{$.Name}}By{{joinFieldNames $idx.Fields}}(ctx context.Context, tr fdb.ReadTransaction, dir directory.DirectorySubspace, {{range $i, $f := $idx.Fields}}{{if $i}}, {{end}}{{$f.Name}} {{$f.Type}}{{end}}) ([]*{{$.Name}}, error) {
	return r.store.Get{{$.Name}}By{{joinFieldNames $idx.Fields}}(ctx, tr, dir, {{range $i, $f := $idx.Fields}}{{if $i}}, {{end}}{{$f.Name}}{{end}})
}
{{end}}

{{range .Fields}}
{{if .MutationValue}}
func (r *{{$.Name | lower}}Repository) {{if eq .MutationValue 2}}Add{{else if eq .MutationValue 12}}Max{{else if eq .MutationValue 13}}Min{{end}}{{$.Name}}{{.Name}}(ctx context.Context, tr Transaction, dir directory.DirectorySubspace, {{range $.PrimaryKeyFields}}{{.Name}} {{.Type}}, {{end}} val {{.Type}}) error {
	return r.store.{{if eq .MutationValue 2}}Add{{else if eq .MutationValue 12}}Max{{else if eq .MutationValue 13}}Min{{end}}{{$.Name}}{{.Name}}(ctx, tr, dir, {{range $.PrimaryKeyFields}}{{.Name}}, {{end}} val)
}
{{end}}
{{end}}
`
