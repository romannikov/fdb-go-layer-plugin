...
func (r *{{.Name | lower}}Repository) Get(ctx context.Context, tr fdb.ReadTransaction, dir directory.DirectorySubspace, pk {{if eq (len .PrimaryKeyFields) 1}}{{(index .PrimaryKeyFields 0).Type}}{{else if gt (len .PrimaryKeyFields) 1}}{{.Name}}PrimaryKey{{else}}struct{}{{end}}) (*{{.Name}}, error) {
	...
	key := dir.Pack(tuple.Tuple{typeID, {{if eq (len .PrimaryKeyFields) 1}}pk{{else if gt (len .PrimaryKeyFields) 1}}{{range $i, $f := .PrimaryKeyFields}}{{if $i}}, {{end}}pk.{{$f.Name}}{{end}}{{end}}})
	...
	{{if .Mutation}}
	{
		fieldKey := dir.Pack(tuple.Tuple{typeID, {{if eq (len $.PrimaryKeyFields) 1}}pk{{else if gt (len $.PrimaryKeyFields) 1}}{{range $i, $f := $.PrimaryKeyFields}}{{if $i}}, {{end}}pk.{{$f.Name}}{{end}}{{end}}, "f", {{.Number}}})
