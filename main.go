...
func ProcessMessage(message *protogen.Message, msgOptions proto.Message) *Message {
			...
			case []*annotationspb.SecondaryIndex:
				for _, idx := range v {
					...
					secondaryIndexes = append(secondaryIndexes, SecondaryIndex{
						Fields:         idxFields,
						IsVersionstamp: idx.Versionstamp,
						IsFanOut:       isFanOut,
	...
	hasVersionstampIndex := false
	for _, idx := range secondaryIndexes {
		if idx.IsVersionstamp {
			hasVersionstampIndex = true
			break
		}
	}
	...
	return &Message{
		...
		HasVersionstampIndex: hasVersionstampIndex,
	}
}
...
import (
	{{if .HasVersionstampIndex}}"encoding/binary"{{end}}
	"fmt"
...
func (s *RecordStore) Create{{.Name}}(tr Transaction, dir directory.DirectorySubspace, entity *{{.Name}}) error {
	...
	{{range $idxIndex, $idx := .SecondaryIndexes}}
	{
		...
		pkTpl := tuple.Tuple{ {{range $.PrimaryKeyFields}} entity.{{.Name}}, {{end}} }
		pkBytes := pkTpl.Pack()
...
{{range $idxIndex, $idx := .SecondaryIndexes}}
{{if $idx.IsVersionstamp}}
// Get{{$.Name}}ByVersionstamp retrieves {{$.Name}} entities ordered by versionstamp.
func (s *RecordStore) Get{{$.Name}}ByVersionstamp(tr fdb.ReadTransaction, dir directory.DirectorySubspace, beginVersionstamp []byte, endVersionstamp []byte) ([]*{{$.Name}}, error) {
	typeID, err := s.get{{$.Name}}TypeID()
	if err != nil {
		return nil, err
	}
