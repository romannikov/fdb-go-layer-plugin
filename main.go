...
type {{.Name}}Repository interface {
	...
	BatchGet{{.Name}}(tr fdb.ReadTransaction, dir directory.DirectorySubspace, ids []tuple.Tuple) (map[string]*{{.Name}}, error)
	List{{.Name}}(tr fdb.ReadTransaction, dir directory.DirectorySubspace, opts {{.Name}}PaginationOptions) (*{{.Name}}PaginatedResult, error)
	{{range $idxIndex, $idx := .SecondaryIndexes}}
	Get{{$.Name}}By{{joinFieldNames $idx.Fields}}(tr fdb.ReadTransaction, dir directory.DirectorySubspace, {{range $i, $f := $idx.Fields}}{{if $i}}, {{end}}{{$f.Name}} {{$f.Type}}{{end}}) ([]*{{$.Name}}, error)
	{{end}}
	{{range .Fields}}
	{{if .MutationValue}}
	{{if eq .MutationValue 2}}Add{{else if eq .MutationValue 12}}Max{{else if eq .MutationValue 13}}Min{{end}}{{$.Name}}{{.Name}}(tr Transaction, dir directory.DirectorySubspace, {{range $.PrimaryKeyFields}}{{.Name}} {{.Type}}, {{end}} val {{.Type}}) error
	{{end}}
	{{end}}
}
...
func (r *{{.Name | lower}}Repository) Create(tr Transaction, dir directory.DirectorySubspace, entity *{{.Name}}) error {
	return r.store.Create{{.Name}}(tr, dir, entity)
}

func (r *{{.Name | lower}}Repository) Get(tr fdb.ReadTransaction, dir directory.DirectorySubspace, pk {{if eq (len .PrimaryKeyFields) 1}}{{(index .PrimaryKeyFields 0).Type}}{{else if gt (len .PrimaryKeyFields) 1}}{{.Name}}PrimaryKey{{else}}struct{}{{end}}) (*{{.Name}}, error) {
	return r.store.Get{{.Name}}(tr, dir, {{if eq (len .PrimaryKeyFields) 1}}pk{{else if gt (len .PrimaryKeyFields) 1}}{{range $i, $f := .PrimaryKeyFields}}{{if $i}}, {{end}}pk.{{$f.Name}}{{end}}{{end}})
}

func (r *{{.Name | lower}}Repository) Set(tr Transaction, dir directory.DirectorySubspace, entity *{{.Name}}) error {
	return r.store.Set{{.Name}}(tr, dir, entity)
}

func (r *{{.Name | lower}}Repository) Delete(tr Transaction, dir directory.DirectorySubspace, pk {{if eq (len .PrimaryKeyFields) 1}}{{(index .PrimaryKeyFields 0).Type}}{{else if gt (len .PrimaryKeyFields) 1}}{{.Name}}PrimaryKey{{else}}struct{}{{end}}) error {
	return r.store.Delete{{.Name}}(tr, dir, {{if eq (len .PrimaryKeyFields) 1}}pk{{else if gt (len .PrimaryKeyFields) 1}}{{range $i, $f := .PrimaryKeyFields}}{{if $i}}, {{end}}pk.{{$f.Name}}{{end}}{{end}})
}

func (r *{{.Name | lower}}Repository) BatchGet{{.Name}}(tr fdb.ReadTransaction, dir directory.DirectorySubspace, ids []tuple.Tuple) (map[string]*{{.Name}}, error) {
	return r.store.BatchGet{{.Name}}(tr, dir, ids)
}

func (r *{{.Name | lower}}Repository) List{{.Name}}(tr fdb.ReadTransaction, dir directory.DirectorySubspace, opts {{.Name}}PaginationOptions) (*{{.Name}}PaginatedResult, error) {
	return r.store.List{{.Name}}(tr, dir, opts)
}

{{range $idxIndex, $idx := .SecondaryIndexes}}
func (r *{{$.Name | lower}}Repository) Get{{$.Name}}By{{joinFieldNames $idx.Fields}}(tr fdb.ReadTransaction, dir directory.DirectorySubspace, {{range $i, $f := $idx.Fields}}{{if $i}}, {{end}}{{$f.Name}} {{$f.Type}}{{end}}) ([]*{{$.Name}}, error) {
	return r.store.Get{{$.Name}}By{{joinFieldNames $idx.Fields}}(tr, dir, {{range $i, $f := $idx.Fields}}{{if $i}}, {{end}}{{$f.Name}}{{end}})
}
{{end}}

{{range .Fields}}
{{if .MutationValue}}
func (r *{{$.Name | lower}}Repository) {{if eq .MutationValue 2}}Add{{else if eq .MutationValue 12}}Max{{else if eq .MutationValue 13}}Min{{end}}{{$.Name}}{{.Name}}(tr Transaction, dir directory.DirectorySubspace, {{range $.PrimaryKeyFields}}{{.Name}} {{.Type}}, {{end}} val {{.Type}}) error {
	return r.store.{{if eq .MutationValue 2}}Add{{else if eq .MutationValue 12}}Max{{else if eq .MutationValue 13}}Min{{end}}{{$.Name}}{{.Name}}(tr, dir, {{range $.PrimaryKeyFields}}{{.Name}}, {{end}} val)
}
{{end}}
{{end}}
`
